...
if __name__ == '__main__':
    ...
    for dirpath, dirs, files in os.walk(files_path):
        ...
        if len(files) != 0:
            ...
            # Calculate the difference in time and in uptime, and then compare
            # time
            timestamp_diff = timestamp[-1] - timestamp[0]
            # uptime
            uptime_diff = round(uptime[-1] - uptime[0])
            print(f'Timestamp delta is {timestamp_diff}')
            print(f'Uptime delta is {uptime_diff}')

            print(f'End of {os.path.basename(dirpath)} folder')
            print('')
